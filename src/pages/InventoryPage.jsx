  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.scroll}>
        {/* Staples section when not filtering */}
        ...
        <Modal onClose={() => setEditingItem(null)}>
          <ItemForm
            ...
            onSave={async (updates) => {
              await onUpdateInventoryItem(editingItem.id, updates)
              setEditingItem(null)
            }}
            onDelete={async () => {
              await onDeleteInventoryItem(editingItem.id)
              setEditingItem(null)
            }}
            ...
            {[...sections].sort((a, b) => a.sort_order - b.sort_order).map((section) =>
              editingSection?.id === section.id ? (
                ...
                <button key={section.id} style={styles.aisleRow} onClick={() => setEditingSection(section)}>
                  ...
                  <span style={styles.aisleMeta}>
                    {(itemsBySection.get(section.id) || []).length} items · #{section.sort_order}
                  </span>
                </button>
              )
            )}
...
function InventoryRow({ item, inList, onAdd, onEdit }) {
  return (
    <div style={styles.row}>
      ...
      <button
        style={{ ...styles.addBtn, background: inList ? 'var(--sage)' : 'var(--terracotta)' }}
        onClick={() => onAdd(item)}
      >
        {inList ? '✓' : '+'}
      </button>
      <button style={styles.editBtn} onClick={onEdit} aria-label={`Edit ${item.name}`}>
        ✎
      </button>
    </div>
  )
...
function ItemForm({ item, sections, onSave, onDelete, onCancel }) {
  ...
  async function handleSave() {
    ...
    await onSave({
      ...
      price_updated_at: (!isEdit || priceChanged) && price ? new Date().toISOString() : (item?.price_updated_at || null),
      ...
      {confirmDelete ? (
        <div style={styles.deleteConfirm}>
          <p style={{ fontSize: 14, margin: '0 0 12px' }}>
            {itemCount > 0
              ? `${itemCount} item${itemCount !== 1 ? 's' : ''} still in this aisle. Reassign them first.`
              : 'Delete this aisle?'}
          </p>
          <div style={styles.formActions}>
            <button style={styles.modalCancel} onClick={() => setConfirmDelete(false)}>Cancel</button>
...
const styles = {
  ...
  editBtn: { border: 'none', background: 'none', color: 'var(--charcoal-soft)', fontSize: 18, padding: '0 4px', flexShrink: 0 },
  ...
  aisleRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid var(--line)', background: 'none', border: 'none', width: '100%', textAlign: 'left', borderBottom: '1px solid var(--line)' },
